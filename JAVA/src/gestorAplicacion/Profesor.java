...
public class Profesor extends Registro {
  ...
  public Profesor(String nombre, String correo, String nombreUsuario, String clave, String documento,
      ArrayList<CursoProfesor> listaCursos, Facultades facultad) {
  ...
  public boolean agregarNota(Estudiante estudiante, Curso curso, int indiceNota, int indiceTipoNota, int nota) {
    ...
    // TODO: mire si la nota es valdia, crear metodo estatico para validar de forma
    // general
    return true;
  }
  ...
  public void agregarCurso(ArrayList<Curso> listaCursos, ArrayList<String> listaHorarios) { // Método único para crear
                                                                                            // profesores en el menú
    for (int x = 0; x < listaCursos.size(); x++) {
      Curso curso = listaCursos.get(x);
      String horario = listaHorarios.get(x);
      CursoProfesor cp = new CursoProfesor(curso.getNombre(), curso.getId(), (short) curso.getCreditos(),
          curso.getNumeroParciales(), curso.getListaPorcentajes(), curso.getFacultad(), horario);
